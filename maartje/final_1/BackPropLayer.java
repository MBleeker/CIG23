...
class BackPropLayer {
	...
	public void computeDelta(BackProp bp, Matrix target) {
		...
		if (this.aLayer.getLayerType() == 3) {
			...
			// I am not sure about this step: I am not sure wheter we should use the inputVector of the current layer --> MAARTJE: Slides say: output current nodes --> Maar Andrew doet het weer anders
			this.gradient = this.deltaMatrix.times(this.aLayer.getInputVector().transpose());
			temp = null;
		}
