...
public class EATorcs {
    ...
    // constants
    private static double EVOL_TIME_SPAN = 20; // in seconds, how much time does an individual have the time to show his/her performance?
    private static int TOTAL_GENERATIONS = 10;
    private static int POPULATION_SIZE = 12;
    ...
    private static int MAX_COMPETITORS = 4;  // must be a value that is dividable by 2 without residual value
    ...
    public void augmentSurvivors(){

        int missing_survivors = (POPULATION_SIZE /2) - this.survivors.size();
        for (int j =0; j < missing_survivors; j++){
            String [] survivor_genome = new String[2];
            // hold nn-steering file-name of this driver in order to clone the network
            survivor_genome[0] = this.mem_files[0][Integer.parseInt(survivors.get(j).driverID) - 1];
            survivor_genome[1] = this.mem_files[1][Integer.parseInt(survivors.get(j).driverID) - 1];
            // does not matter at this moment which ID the new survivor gets
            DefaultDriver dd = new DefaultDriver(Integer.toString(POPULATION_SIZE+j+1), EVOL_TIME_SPAN);
        ...
        for (int j = 0; j < this.population.size(); j++) {
            ...
            if (i == MAX_COMPETITORS || (this.population.size() == total )) {
                ...
                if ( this.population.size() - total < MAX_COMPETITORS ) {
                    ...
                    System.out.println("============= Next tournament with ================");
    ...
    public void makeNewGeneration(ArrayList<DefaultDriver> survivors) {
        ...
        // first check whether at least half of the population survived, if not arbitrary clone survivors
        if ((survivors.size() < POPULATION_SIZE /2)) {
            this.augmentSurvivors();
        }
        ...
        for (int i=0; i<survivors.size(); i++) {
            ...
            if (family_members <= (POPULATION_SIZE / 2)) {
                family_members++;
                // create new driver
